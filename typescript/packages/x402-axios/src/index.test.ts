...
import { evm, PaymentRequirements, ChainIdToNetwork, Signer, MultiNetworkSigner } from "@peezy.tech/x402/types";
import { withPaymentInterceptor } from "./index";

// Mock the createPaymentHeader function
vi.mock("@peezy.tech/x402/client", () => ({
...
describe("withPaymentInterceptor()", () => {
  ...
  beforeEach(async () => {
    ...
    const { selectPaymentRequirements } = await import("@peezy.tech/x402/client");
  ...
  it("should handle 402 errors and retry with payment header", async () => {
    ...
    const { createPaymentHeader, selectPaymentRequirements } = await import("@peezy.tech/x402/client");
  ...
  it("should reject if payment header creation fails", async () => {
    ...
    const { createPaymentHeader } = await import("@peezy.tech/x402/client");
...
describe("withPaymentInterceptor() - SVM and MultiNetwork", () => {
  ...
  it("passes [solana, solana-devnet] for SVM-only signers", async () => {
    vi.doMock("@peezy.tech/x402/types", async () => {
      const actual = await vi.importActual("@peezy.tech/x402/types");
      return {
        ...actual,
    ...
    vi.doMock("@peezy.tech/x402/client", () => ({
